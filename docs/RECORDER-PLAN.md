...
| 5 | The platform spawns the recorder: gated by the tenant's plan and rented machines, not by a tool inside the tenant's namespace | **done · v349 + provisioner 20c3f61** (2026-09-18) | — |
...
## Phase 5 — proof (2026-09-18, GB v349 + provisioner 20c3f61)

- **Provisioner** (`src/recorders.js`, routes in `src/index.js`, `listPodsByLabel`/`deploymentImage` in `src/k8s.js`): `POST/GET/DELETE /api/recorders` behind `requireRecorderAuth` — a per-tenant recorder token (`HMAC(AGENTS_SSO_SECRET|JWT_SECRET, "recorder:<tenant>")`, `x-recorder-token` + `x-pod-name`) or the usual human/internal auth. `policyOf(plan)`: free = 1 at a time / 720p / 60 min; pro or a rented machine = 3 / up to 1080p60 / 720 min; superadmin unlimited. 403 `plan` (quality, length), 409 `capacity` + `retryAfterSec` (already running), 503 when the tenant's GB image cannot be read or the cluster refuses. The Job (`rec-<tenant>-<recordingId>`, namespace `agents-system`, labels `app-cluster/job-type=recorder`, `app-cluster/pod=<tenant>`) runs the tenant's OWN GB image in recorder mode, requests 1 GiB / 0.5 CPU, limits 6 GiB / 4 CPU, scratch 20 GiB, no service-account token; a tenant with a dedicated node gets `nodeSelector dedicated-tenant=<tenant>` AND the matching NoSchedule toleration. 7 unit tests (fake k8s) + 3 route tests (auth ordering) + a manifest test (the wiring lands in GB's env and secret).
- **GB install wiring**: `installNamespaceTool('ghost-browser')` passes `wiring` → `RECORDER_PLATFORM_URL` (plain), `RECORDER_PLATFORM_TENANT` (plain), `RECORDER_PLATFORM_TOKEN` (in GB's own secret). The live tenant was wired by `kubectl set env` (the token as a plain env for now); a re-connect of the tool from the Tools tab moves it into the secret.
- **GB side**: `chooseRemote()` picks the platform remote when the three envs are set (boot line "recorders are spawned by the platform"); the pod reaches GB by `ghost-browser-pods.<ns>.svc.cluster.local` (the platform namespace is allowed in by the tenant network policy).
- **Live**: a 1-minute ask → the platform created `rec-mavicpro-fan-rec-mu6kb251-i8gf0` in `agents-system`; the pod took the handoff (1153 cookies from the live YouTube session), dismissed the consent wall, recorded 60 s full screen with sound (mean −24.7 dB / max −0.4 dB) and pushed 6 segments back; the tenant's quota stayed at 8640 MiB throughout. A bad request at the door got 400 (not 401): the tenant's token opens it.
- Left for later: the platform charging recording minutes (the Job's labels carry the tenant, the minutes are in GB's journals), the capacity controller consuming `demand`, and BYON clusters (a remote k3s) which this door does not reach yet — there GB's own in-namespace Job remains the path.
