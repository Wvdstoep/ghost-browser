...
| 2 | The on-device crawl | **READER DONE · proven against a stubbed device** (2026-09-21) — `src/deviceThread.js` reads author, participants and profile slugs from a post, PRESSING the reveal-comments control first (bounded) because on LinkedIn mobile web the comments are frequently not in the document until something is pressed. It reports `pressed`, `truncated` and `nothingToExpand`, so a zero that has looked reads differently from a zero that has not — the silence that would otherwise stop a reply desk without anybody noticing. **The reader lives on the CLUSTER**, because the phone routes each call a hardcoded `window.__gb.<fn>` in the APK assets with no eval, so a phone-side reader would need an app release per iteration of DOM work. `analyze` already returns hrefs, and a LinkedIn slug IS the href. OPEN: the thread is not yet handed to the cluster ingest that drafts and gates it | A LinkedIn post/notification is read on the phone (its login, its IP, no Cloudflare loop), the thread returns to the cluster, and the cluster ingests → drafts → gates it exactly like Facebook |
| 3 | Real Cloudflare-wall detection | **DONE · proven** (2026-09-21) — `src/siteWalls.js` learns a walled host as data with its evidence; the sensor sits in `diagnostics.attach`, which pool.js already installs on every page, so it covers all fourteen goto call sites at once. `challengeOf` reads headers only (`cf-mitigated`, or 403/503 served by cloudflare) and a 403 from anything else stays an ordinary refusal. A wall clears after three clean loads. Learned live, then corrected live: a redirect loop is now told from browsing BY TIME (repeats inside 20s), because `distinct <= 3` cannot separate a redirect bouncing between two addresses from a session visiting two pages — my own probing was recorded as a wall. Readable at `GET /v1/site-walls`, removable per host | A site that redirect-loops or shows a Cloudflare challenge from the cluster is auto-flagged needsDevice; the flag is data, not a hardcode; LinkedIn is detected, not listed |
| 4 | The scheduler dispatches to the ring | **DONE for watcher passes and for the job GATE** (2026-09-21) — the scheduler TODO is gone: a gated watcher pass now runs on the device via `runPassOnDevice`, and no device still means wait, never the cluster. `enqueue`/`runCommand` were factored out of the HTTP route so the scheduler no longer has to call itself over HTTP. `ringGate` also refuses JOBS for a gated surface before a model is touched or a session claimed, from the ADDRESSES IN THE GOAL rather than the role, because one profile serves surfaces with opposite requirements. A MISSING LOGIN is now a second reason to need a device, per SURFACE: `searchconsole` borrows the `google` profile and declares `needsLogin`, so learning it cannot be read routes Search Console and leaves web search on the cluster. OPEN: an agent JOB still does not RUN on the phone — the agent drives a Playwright page, and giving it the device as a backend is the remaining work | A due LinkedIn watch runs its pass on the connected phone end to end; an approved reply to a gated platform posts from the phone; no device → clean waiting-device, never the cluster |
