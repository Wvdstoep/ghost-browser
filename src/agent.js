...
const TOOLS = [
  ...
  { type: 'function', function: { name: 'upload_file', description: 'Put a STORED file (a video clip or image saved earlier by download_file) onto the current page — sets the site\'s file input from the asset id. Open the upload / "Add media" control first so the file input exists, then call this. Use it to feed an image into a video generator, or clips into an editor.', parameters: { type: 'object', properties: { assetId: { type: 'string', description: 'the asset id a download_file step returned' } }, required: ['assetId'] } } },
