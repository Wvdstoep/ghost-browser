...
function mountDeviceHub(app, authed) {
  const devices = new Map(); // id -> {name, owner, queue:[], pollWaiters:[], resultWaiters:Map, lastSeen}
  let seq = 0;
  ...
  app.post('/v1/device/register', authed, (req, res) => {
    ...
    if (!d) d = { name: '', owner: ownerOf(req), queue: [], pollWaiters: [], resultWaiters: new Map(), lastSeen: 0 };
