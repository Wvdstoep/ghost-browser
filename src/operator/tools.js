...
function registerOperatorTools(reg, ctx) {
  ...
  R('gb_busy', 'Who holds the browser right now (running watcher passes, a poster). Nothing that needs a profile should start while this is non-empty.', obj({}), async () => ({ running: [...ctx.runningWatchers] }), { repeatable: true });
